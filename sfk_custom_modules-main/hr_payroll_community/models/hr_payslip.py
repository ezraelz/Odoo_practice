...
class HrPayslip(models.Model):
    ...
    def get_contract(self, employee, date_from, date_to):
        """
        Return the employee's active contracts for the given period.
        Only picks contracts that are in running/open state.
        """
        if not employee:
            return []

        # Clause 1: Contract ends during period
        clause_1 = ['&', ('date_end', '<=', date_to), ('date_end', '>=', date_from)]
        # Clause 2: Contract starts during period
        clause_2 = ['&', ('date_start', '<=', date_to), ('date_start', '>=', date_from)]
        # Clause 3: Contract spans entire period (starts before, ends after or never ends)
        clause_3 = ['&', ('date_start', '<=', date_from), '|', ('date_end', '=', False), ('date_end', '>=', date_to)]

        domain = [
                     ('employee_id', '=', employee.id),
                     ('state', '=', 'open'),  # only running/open contracts
                     '|', '|'
                 ] + clause_1 + clause_2 + clause_3

        contracts = self.env['hr.contract'].search(domain, order='date_start asc')
